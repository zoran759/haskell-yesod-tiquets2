...
globalsForm master langs mbFDG = renderTable $ FormDadesGlobals
        <$> (formToAForm $ mreq comissióField (fieldSettingsLabel MsgFrmGlobalsComissió) (fdgComissió <$> mbFDG) >>= (return . fmap UF.vistaAmbEstrellaL))
        <*> (formToAForm $ mreq textField (fieldSettingsLabel MsgFrmGlobalsAdmin) (fdgAdmin1 <$> mbFDG) >>= (return . fmap UF.vistaAmbEstrellaL))
